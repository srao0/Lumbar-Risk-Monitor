...
Append a 4-bit status mask to each CSV row and the BLE packet, where bit i = IMU i alive.
...
10 kΩ), pulse it low ~1 ms inside the recovery block for a hard mux reset. That is more
