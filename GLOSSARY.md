...
Terms and acronyms used across this repository, in plain language.

## System and anatomy
- **PLTU**: the four-IMU placement model. **P**elvis, **L**3 (lumbar), **T**12–L1 (thoracolumbar), **T**4–T6 (upper thoracic). The pelvis IMU is the reference, and the others measure segment orientation relative to it.
- **IMU**: Inertial Measurement Unit. Combines accelerometer and gyroscope (and magnetometer) to estimate orientation. Here, the BNO/ICM-class sensors sampled at 100 Hz.
...
- **LOSO**: Leave-One-Subject-Out cross-validation. Train on everyone *except* one person, then test on them (measures generalisation to a new person).
