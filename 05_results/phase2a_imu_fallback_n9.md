...
- Reduced ≥ primary on *both* within-CV and LOSO: the two-IMU set matches or beats the full set, which supports the recommendation.
...
> The main comparison plots (`01_within_primary_vs_reduced`, `02_loso_primary_vs_reduced`, `03_loso_scatter_primary_vs_reduced`, `05_aggregate_auc_summary`, `PR_curves_within/loso`, `confusion_matrices_aggregate`) were **regenerated on the corrected-QC tier (2026-06-12)** and match the canonical numbers. Warning: the cohort/auxiliary plots (`plot_01…plot_05_*`, `feature_importance_RF`, `roc_*`, `per_session_auc_RF`) were **not** regenerated and predate the re-freeze, so they are illustrative only; cite `frozen_numbers/frozen_numbers_sheet.md` for values.
...
- The LOSO advantage is real but **not significant**, so the claim is "equal accuracy from half the sensors," not "better."
